...
export default function DescriptionListTable({
  ...
  const columns = [
...

    {
      ...
      key: 'id',
      align: 'center',
      render: (descriptionId: string) => <Input {...register(descriptionId)} type="text" />
    },
  ];

  const onSubmit = (values: Record<string, string>) => {
    const updatedDescriptions = descriptionList.reduce((acc: CustomDescription[], curr) => {
      if (values[curr.id] !== '') {
        acc = acc.concat([{
  ...
  return <>
  <div className="flex justify-center lg:justify-end  items-center my-6">
