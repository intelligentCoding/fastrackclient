...
export default function ManifestList() {
    ...
    const { manifestList, loading: fetchingServices, error } = useManifestListQuery();
        ...
        <>
            {manifestList.length ?
                manifestList.map((list, index) => {
                    return (
                        <div key={index} className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8 ">
